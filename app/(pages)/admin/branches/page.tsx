...
const BranchesPage: React.FC = () => {
  ...
  const handleDeleteBranch = async (branchId: string) => {
    setLoading(true);
    try {
      const response = (await branchApi.deleteBranch({
        branchId,
      })) as DeleteResponse;
      if (response.success) {
        ...
        toast.error("Failed to delete branch", {
          description:
            response.message || "An error occurred while deleting the branch.",
        });
      }
    } catch (err) {
      if (axios.isAxiosError(err)) {
        ...
        <DialogContent>
          ...
          {selectedBranch && (
            <div className="space-y-2">
              ...
              <p>
                <strong>Email:</strong> {selectedBranch.branchEmail || "N/A"}
              </p>
              <p>
                <strong>Services Offered:</strong>{" "}
                {selectedBranch.servicesOffered.join(", ")}
              </p>
              <p>
        ...
        <DialogContent>
          ...
          <div className="mt-4 justify-end flex gap-3">
            ...
            <Button
              ...
              disabled={loading}
              onClick={() =>
                selectedBranch && handleDeleteBranch(selectedBranch.id)
              }
            >
