...
const EditServiceHistoryPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchServiceHistory = async () => {
            setLoading(true);
            try {
                const response = (await serviceHistoryApi.getServiceHistoryById({serviceHistoryId})) as ApiResponse;
                if (response.success) {
                    setFormData({
                        serviceHistoryId,
                    ...
                    toast.error("Failed to load service record", {description: response.message});
                }
            } catch (error) {
    ...
    const handleUpdate = async () => {
        setLoading(true);
        try {
            const response = (await serviceHistoryApi.updateServiceHistory(formData)) as ApiResponse;
            if (response.success) {
                toast.success("Service record updated successfully!");
                router.push("/operations/service-history");
            } else {
                toast.error("Failed to update service record", {description: response.message});
            }
        } catch (err) {
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                    <Button variant="outline" onClick={() => router.push("/operations/service-history")}>Cancel</Button>
