...
const ServiceHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchServiceHistories = async () => {
      ...
      try {
        const response =
          (await serviceHistoryApi.getServiceHistories()) as ApiResponse;
        if (response.success) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          onClick={() => router.push("/operations/service-history/create")}
        >
          + Add Record
        </Button>
      </div>
                      ...
                      <PopoverContent className="w-40 p-2 bg-white border rounded-md shadow-lg">
                        <div className="flex flex-col gap-1">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                `/operations/service-history/${history.id}/edit`
                              )
                            }
                          >
          ...
          {selectedHistory && (
            <div className="space-y-2">
              <h2 className="text-lg font-semibold">
                Service on{" "}
                {new Date(selectedHistory.serviceDate).toLocaleDateString()}
              </h2>
              <p>
