...
const CustomersPage: React.FC = () => {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    toast.error("Failed to load customers", {
                        description: response.message || "An error occurred while fetching customers.",
                    });
                }
            } catch (err) {
    ...
    const handleDeleteCustomer = async (customerId: string) => {
        setLoading(true);
        try {
            const response = (await customerApi.deleteCustomer({customerId}) as DeleteResponse);
            if (response.success) {
                ...
                toast.error("Failed to delete customer", {
                    description: response.message || "An error occurred while deleting the customer.",
                });
            }
        } catch (err) {
            if (axios.isAxiosError(err)) {
                toast.error("Failed to delete customer", {
                    description: err.response?.data?.message || "An error occurred while deleting the customer.",
                });
            } else {
                toast.error("Failed to delete customer", {
                    description: "An unexpected error occurred.",
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                ...
                <Button onClick={() => router.push("/operations/customers/create")} className="px-4 py-2">
                    + Add Customer
                </Button>
            </div>
            ...
            {loading ? (
                ...
                <div className="border rounded-lg shadow-sm overflow-hidden">
                    <Table className="w-full border-collapse">
                        <TableCaption className="py-3 text-gray-500">List of all customers.</TableCaption>
                ...
                <DialogContent>
                    ...
                    {selectedCustomer && (
                        <div className="space-y-2">
                            ...
                            <p><strong>Branch ID:</strong> {selectedCustomer.branchId}</p>
                        </div>
                    )}
                </DialogContent>
            </Dialog>
