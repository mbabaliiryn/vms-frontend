...
const EditCustomerPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchCustomer = async () => {
            setLoading(true);
            try {
                const response = (await customerApi.getCustomerById({customerId})) as ApiResponse;
                if (response.success) {
                    setFormData({
                        customerId,
                    ...
                    toast.error("Failed to load customer", {description: response.message});
                }
            } catch (error) {
    ...
    const handleUpdate = async () => {
        setLoading(true);
        try {
            const response = (await customerApi.updateCustomer(formData)) as ApiResponse;
            if (response.success) {
                toast.success("Customer updated successfully!");
                router.push("/operations/customers");
            } else {
                toast.error("Failed to update customer", {description: response.message});
            }
        } catch (err: unknown) {
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                    <Button variant="outline" onClick={() => router.push("/admin/customers")}>Cancel</Button>
