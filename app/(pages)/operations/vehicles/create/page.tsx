...
const CreateVehiclePage: React.FC = () => {
            ...
            try {
                ...
                if (customerRes.success && branchRes.success) {
                    ...
                    toast.error("Failed to load data", {
                        description: "An error occurred while fetching customers and branches.",
                    });
                }
            } catch (err) {
                if (axios.isAxiosError(err)) {
                    if (err.config?.url?.includes('customers')) {
                        toast.error("Failed to load customers", {
                            description: err.response?.data?.message || "An error occurred while fetching customers.",
                        });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = (await vehicleApi.createVehicle(vehicleData) as { success: boolean, message: string });
            if (response.success) {
        ...
        <div className="p-6 max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <Select onValueChange={(value) => setVehicleData({...vehicleData, customerId: value})}>
                        <SelectTrigger>
                ...
                <div>
                    <Label>Branch</Label>
                    <Select onValueChange={(value) => setVehicleData({...vehicleData, branchId: value})}>
                        <SelectTrigger>
                ...
                <div>
                    <Label>Make</Label>
                    <Input name="make" value={vehicleData.make} onChange={handleChange} required/>
                </div>

                <div>
                    <Label>Model</Label>
                    <Input name="model" value={vehicleData.model} onChange={handleChange} required/>
                </div>
                ...
                <div>
                    ...
                    <Input name="plateNumber" value={vehicleData.plateNumber} onChange={handleChange} required/>
                </div>
