...
const EditVehiclePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchVehicle = async () => {
            setLoading(true);
            try {
                const response = (await vehicleApi.getVehicleById({vehicleId})) as ApiResponse;
                if (response.success) {
                    setFormData({
                        vehicleId: vehicleId,
                    ...
                    toast.error("Failed to load vehicle", {description: response.message});
                }
            } catch (error) {
    ...
    const handleUpdate = async () => {
        setLoading(true);
        try {
            const response = (await vehicleApi.updateVehicle(formData)) as ApiResponse;
            if (response.success) {
                toast.success("Vehicle updated successfully!");
                router.push("/operations/vehicles");
            } else {
                toast.error("Failed to update vehicle", {description: response.message});
            }
        } catch (err: unknown) {
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                    <Button variant="outline" onClick={() => router.push("/operations/vehicles")}>
                        Cancel
                    </Button>
